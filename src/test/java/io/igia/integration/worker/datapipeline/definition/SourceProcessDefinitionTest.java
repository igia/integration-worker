...
public class SourceProcessDefinitionTest {
    ...
    public void testConfigure() throws Exception {
        ...
        RouteBuilder routeBuilder = sourceProcessDefinition.configure(dataPipeline);
        ...
        assertEquals(4, sourceProcessRouteDefinition.getOutputs().size());

        OnExceptionDefinition onExceptionDefinition = (OnExceptionDefinition) sourceProcessRouteDefinition.getOutputs().get(0);
        ...
        TransformDefinition sourceTransformDefinition = (TransformDefinition) sourceProcessRouteDefinition.getOutputs().get(1);
        assertEquals(sourceEndpoint.getTransformers().iterator().next().getData(),
                sourceTransformDefinition.getExpression().getExpressionType().getExpression());

        MarshalDefinition sourceMarshalDefinition = (MarshalDefinition) sourceProcessRouteDefinition.getOutputs().get(2);
        assertEquals("hl7", sourceMarshalDefinition.getDataFormatType().getDataFormatName());

        ToDefinition sourceToDefinition = (ToDefinition) sourceProcessRouteDefinition.getOutputs().get(3);
        assertEquals("direct:Destination8", sourceToDefinition.getUri());
    }
