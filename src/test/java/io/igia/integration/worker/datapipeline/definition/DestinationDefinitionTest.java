...
public class DestinationDefinitionTest {
    ...
    public void testConfigure() throws Exception {
        ...
        RouteBuilder routeBuilder = destinationDefinition.configure(dataPipeline);
        routeBuilder.configure();

        assertEquals(1, routeBuilder.getRouteCollection().getRoutes().size());

        RouteDefinition destinationRouteDefinition = routeBuilder.getRouteCollection().getRoutes().get(0);
        assertEquals("direct:Destination8", destinationRouteDefinition.getInputs().get(0).getEndpointUri());

        assertEquals(4, destinationRouteDefinition.getOutputs().size());

        SetPropertyDefinition destinationProperty1 = (SetPropertyDefinition) destinationRouteDefinition.getOutputs().get(0);
        assertEquals("dataPipelineId", destinationProperty1.getPropertyName());
        assertEquals("8", destinationProperty1.getExpression().getExpression());

        SetPropertyDefinition destinationProperty2 = (SetPropertyDefinition) destinationRouteDefinition.getOutputs().get(1);
        assertEquals("endpointName", destinationProperty2.getPropertyName());
        assertEquals("Destination", destinationProperty2.getExpression().getExpression());

        OnExceptionDefinition onExceptionDefinition = (OnExceptionDefinition) destinationRouteDefinition.getOutputs().get(2);
        ...
        SplitDefinition destinationSplitDefinition = (SplitDefinition) destinationRouteDefinition.getOutputs().get(3);
        TokenizerExpression destinationTokenizerExpression = (TokenizerExpression) destinationSplitDefinition.getExpression();
        assertEquals("MSH", destinationTokenizerExpression.getToken());

        FilterDefinition destinationFilterDefinition = (FilterDefinition) destinationSplitDefinition.getOutputs().get(2);
        assertEquals(destinationEndpoint.getFilters().iterator().next().getData(),
                destinationFilterDefinition.getExpression().getExpression());

        ToDefinition destinationToDefinition = (ToDefinition) destinationFilterDefinition.getOutputs().get(0);
