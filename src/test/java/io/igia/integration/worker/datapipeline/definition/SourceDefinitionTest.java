...
public class SourceDefinitionTest {
    ...
    public void testConfigure() throws Exception {
        ...
        Mockito.when(sourceRouter.redirect(ArgumentMatchers.any(SourceEndpoint.class),ArgumentMatchers.anyString())).thenReturn("mllp://localhost:9000");
        ...
        RouteDefinition sourceRouteDefinition = routeBuilder.getRouteCollection().getRoutes().get(0);
        assertEquals("mllp://localhost:9000", sourceRouteDefinition.getInputs().get(0).getEndpointUri());

        assertEquals(6, sourceRouteDefinition.getOutputs().size());

        SetPropertyDefinition sourceProperty1 = (SetPropertyDefinition) sourceRouteDefinition.getOutputs().get(0);
        assertEquals("dataPipelineId", sourceProperty1.getPropertyName());
        assertEquals("8", sourceProperty1.getExpression().getExpression());

        SetPropertyDefinition sourceProperty2 = (SetPropertyDefinition) sourceRouteDefinition.getOutputs().get(1);
        assertEquals("endpointName", sourceProperty2.getPropertyName());
        assertEquals("Source", sourceProperty2.getExpression().getExpression());

        SetPropertyDefinition sourceProperty4 = (SetPropertyDefinition) sourceRouteDefinition.getOutputs().get(2);
        assertEquals("messageType", sourceProperty4.getPropertyName());
        assertEquals("ALL", sourceProperty4.getExpression().getExpression());

        WireTapDefinition<?> wireTapDefinition = (WireTapDefinition<?>) sourceRouteDefinition.getOutputs().get(3);
        assertEquals("seda:ALLMESSAGES", wireTapDefinition.getUri());

        OnExceptionDefinition onExceptionDefinition = (OnExceptionDefinition) sourceRouteDefinition.getOutputs().get(4);
        ...
        SplitDefinition sourceSplitDefinition = (SplitDefinition) sourceRouteDefinition.getOutputs().get(5);
        TokenizerExpression sourceTokenizerExpression = (TokenizerExpression) sourceSplitDefinition.getExpression();
        assertEquals("MSH", sourceTokenizerExpression.getToken());

        UnmarshalDefinition sourceUnmarshalDefinition = (UnmarshalDefinition) sourceSplitDefinition.getOutputs().get(1);
        assertEquals("hl7", sourceUnmarshalDefinition.getDataFormatType().getDataFormatName());

        FilterDefinition sourceFilterDefinition = (FilterDefinition) sourceSplitDefinition.getOutputs().get(2);
        assertEquals(sourceEndpoint.getFilters().iterator().next().getData(),
                sourceFilterDefinition.getExpression().getExpression());

        ToDefinition sourceToDefinition = (ToDefinition) sourceFilterDefinition.getOutputs().get(0);
